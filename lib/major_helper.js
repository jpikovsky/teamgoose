...
exports.formatCourseInfo = (reqs, courses) => {
	...
	for(var i = 0; i<reqs.length; i++){
		...
		var num = reqs[i].num;
		var num_courses = reqs[i].num_courses;
		if(req_num in info){
			...
			if(courses && info[req_num].req_satisfied == 'No'){
				for(var j=0; j<courses.length; j++){
					if(courses[j].dept == dept && courses[j].num == num){
						info[req_num].num_taken++;
						if(info[req_num].num_taken >= info[req_num].num_courses){
