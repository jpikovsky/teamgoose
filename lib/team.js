 ...
 */
function member(user, fname, lname, year, writeup) {
	return {
		user: user,
		fname: fname,
		lname: lname,
...
var team = [
	member('tcontois', 'Tim', 'Contois', 'senior', reader.readFileSync("./write_up/tim_writeup.txt", 'utf8')),
	member('alassall', 'Alex', 'Lassalle', 'senior', reader.readFileSync("./write_up/alex_writeup.txt", 'utf8')),
	member('agramign', 'Andrew', 'Gramigna', 'senior', reader.readFileSync("./write_up/andrew_writeup.txt", 'utf8')),
	member('jpikovsk', 'Josh', 'Pikovsky', 'senior', reader.readFileSync("./write_up/josh_writeup.txt", 'utf8')),
	member('rbcondon', 'Riley', 'Condon', 'senior', reader.readFileSync("./write_up/riley_writeup.txt", 'utf8')),
	member('nmurali', 'Niketh', 'Murali', 'junior', reader.readFileSync("./write_up/niketh_writeup.txt", 'utf8'))
];
 ...
 */
function copy(memberToCopy) {
	return member(memberToCopy.user, memberToCopy.fname, memberToCopy.lname, memberToCopy.year, memberToCopy.writeup);
}
...
function one(user) {
	...
	if(typeof user !== 'string'){
		return result(false, 'param user not of type string', []);
	}
	var mem = find(user);
	if(mem === null){
		return result(false, 'team member not found', []);
	}
	return result(true, 'team member found', [mem]);
}
